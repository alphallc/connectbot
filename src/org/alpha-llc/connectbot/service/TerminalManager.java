...
package org.alpha-llc.connectbot.service;
...
import org.alpha-llc.connectbot.transport.TransportFactory;
	...
	private void disconnectAll(final boolean immediate, boolean onlyRemote) {
		...
		if (tmpBridges != null) {
			...
			for (int i = 0; i < tmpBridges.length; i++) {
				if (!onlyRemote || !(tmpBridges[i].transport instanceof org.alpha-llc.connectbot.transport.Local))
