...
package org.alphallc.sshit.util;
...
////		Log.i(":::::::::::::PUBLIC::::::::FMT::::::", pubKeySpec.getFormat());
	...
	static PublicKey recoverPublicKey(KeyFactory kf, PrivateKey priv)
		...
		if (priv instanceof RSAPrivateKey) {
			Log.i(TAG, "We're on Android 4.3+. So, due to bug #58231, we assume public exponent is 65537");
			RSAPrivateKey rsaPriv = (RSAPrivateKey) priv;
			BigInteger modulus = rsaPriv.getModulus();

			BigInteger pub_exponent = (new BigInteger("65537"));
			// ^^ I dislike the hardcode, but:
			// 1) most of the time (if not all of it) it is 65537. Enjoy, lol.
			//    (Alternative recommended exponent is 3, but it is too small
			//    and unusual in use. And anyway, I dislike even two-variants
			//    hardcode. So, let's invent some way to get exponent with Java
			//    tools)
			// 2) Honestly, I absolutelly have no idea, how to really extract
			//    pubkey exponent from PrivateKey in androids >=4.3, because
			//    Google broke API and keeps silence for >1.5 years:
			//    https://code.google.com/p/android/issues/detail?id=58231
			//    if they will fix it, then I will fix that hardcode, but for
			//    now, I'd keep it here, since I wasted tons of time in trying
			//    to do it in proper way.
			//    Many thanks to my microblog-buddy Tishka17 for cheat hint!

			return kf.generatePublic(new RSAPublicKeySpec(modulus, pub_exponent));
        } else if (priv instanceof RSAPrivateCrtKey) {
	...
	public static String convertToOpenSSHFormat(PublicKey pk, String origNickname) throws IOException, InvalidKeyException {
		...
		if (pk instanceof RSAPublicKey) {
			...
			Log.i(TAG, ("ECDSA KeyType: ").concat(keyType));
