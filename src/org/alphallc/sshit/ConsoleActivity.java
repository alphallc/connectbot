...
import org.alphallc.sshit.bean.SelectionArea;
import org.alphallc.sshit.service.PromptHelper;
import org.alphallc.sshit.service.TerminalBridge;
import org.alphallc.sshit.service.TerminalKeyListener;
import org.alphallc.sshit.service.TerminalManager;
import org.alphallc.sshit.util.FileChooser;
import org.alphallc.sshit.util.FileChooserCallback;
import org.alphallc.sshit.util.PreferenceConstants;
import org.alphallc.sshit.util.PubkeyDatabase;
import org.alphallc.sshit.util.PubkeyUtils;
import org.alphallc.sshit.util.TransferThread;
...
public class ConsoleActivity extends Activity implements FileChooserCallback {
	public final static String TAG = "sshIt.ConsoleActivity";
		...
		final GestureDetector gestDetect = new GestureDetector(new GestureDetector.SimpleOnGestureListener() {
			...
			public void onLongPress(MotionEvent e) {
				...
				if (fullScreen == FULLSCREEN_ON) {
					itemList.add(ConsoleActivity.this.getResources().getString(R.string.longpress_disable_full_screen_mode));
				} else {
					itemList.add(ConsoleActivity.this.getResources().getString(R.string.longpress_enable_full_screen_mode));
				}
	...
	private void setupPublicKey(PubkeyBean pubkey) {
		...
		try {
			...
			if (terminal != null && terminal.bridge != null)
				terminal.bridge.injectString(" mkdir -p -m 700 ~/.ssh; echo " + openSSHPubkey + " >> ~/.ssh/authorized_keys # confirm this key by pressing Enter");
